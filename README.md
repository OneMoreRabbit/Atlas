...
- [`provides/`](provides/) — the method seat's responses: answers to needs addressed to @atlas, each with `responds_to:` naming the need (so the raiser's sweep sees it answered). Outbox traffic — commits straight to `main`, untagged.
- [`needs/`](needs/) — the method seat's outbox: findings and asks from @atlas addressed to project arch seats, read in place (every seat clones this repo). **Outbox traffic commits straight to `main`, untagged** — it is communication, not method content, so it is never gated behind a release; an untagged commit on `main` is not a release (§9) and moves no seat's pin. Answered/retired by the raiser like any need.
